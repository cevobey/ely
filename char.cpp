...
#ifdef BOSS_ARENA
	event_cancel(&m_pkBossInfoEvent);
#endif
#ifdef ENABLE_WOLFMAN_CHARACTER
...
#ifdef ENABLE_YANG_LIMIT_SYSTEM
			header.aPriceInfo[idx].llPrice = it->second;
#else
...
void CHARACTER::Disconnect(const char * c_pszReason)
{
	...
	if (GetQuestFlag("dealornodeal.start") > 0)
	{
		SetQuestFlag("dealornodeal.logout",1);
		SetQuestFlag("dealornodeal.start",0);
	}
#endif
...
EVENTFUNC(bossarena_event)
{
	...
	CBossArena::instance().Notification(ch);

	return PASSES_PER_SEC(60);

}

void CHARACTER::StartBossArenaEvent()
{
	if (m_pkBossInfoEvent)
		return;
	...
	m_pkBossInfoEvent = event_create(bossarena_event, info, PASSES_PER_SEC(60));
}
#endif
...
void CHARACTER::RefineAcceMaterials()
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("window_close"));
		return;
	}
...
void CHARACTER::LoadBattlePassInfo() {
	if (!IsPC()) { return; }
	char file_name[256+1];
	snprintf(file_name, sizeof(file_name), "%s/battlepass_players/%s.json", LocaleService_GetBasePath().c_str(), GetName());

	if (const bool exist = std::filesystem::exists(file_name); !exist) {
		return;
	}
	...
	try {
		...
		auto& instance = CNewBattlePassManager::instance();

		json& arr = jf;
		const auto maxSize = instance.GetMissionSize();
		m_BattlePassInfo.vecBattlePass.reserve(maxSize);
        BYTE idx = 1;
		for ( const auto& i : arr) {
			if (idx > maxSize) { break; }

			char szMissionKey[12]{};
			snprintf(szMissionKey, sizeof(szMissionKey), "mission_%u", idx);

			ULDWORD value = 0;
			i.at(szMissionKey).get_to(value);

			const auto missionMaxVal = instance.GetMissionValue(idx - 1); // -> vector index
			const bool isCompleted = value >= missionMaxVal;

			m_BattlePassInfo.vecBattlePass.emplace_back(value, isCompleted);
			++idx;
		}
		m_BattlePassInfo.vecBattlePass.resize(maxSize);
		m_BattlePassInfo.vecBattlePass.shrink_to_fit();
	}
	catch (const std::exception& e) {
		sys_err("LoadBattlePassInfo : %s", e.what());
	}
}

void CHARACTER::CreateBattlePassFile() {
	if (!IsPC()) { return; }
	char file_name[256+1];
	snprintf(file_name, sizeof(file_name), "%s/battlepass_players/%s.json", LocaleService_GetBasePath().c_str(), GetName());
	if (const bool exist = std::filesystem::exists(file_name); !exist)
	{
		try {
			...
			for (BYTE idx = 1; idx <= CNewBattlePassManager::instance().GetMissionSize(); ++idx) {
				char szMissionKey[12]{};
				snprintf(szMissionKey, sizeof(szMissionKey), "mission_%u", idx);
			...
			std::ofstream newFile(file_name, std::ios::trunc | std::ios_base::out);
			newFile << result.dump(4);
			newFile.close();
		}
		catch (const std::exception& e) {
			sys_err("CreateBattlePassFile : %s", e.what());
		}
	}
}

void CHARACTER::UpdateBattlePassMission(const BYTE idx, const ULDWORD value) {
	if (!IsPC()) { return; }
	if (idx >= m_BattlePassInfo.vecBattlePass.size()) { return; }
	auto& vec = m_BattlePassInfo.vecBattlePass[idx];

	if (vec.second) { return; } // complated

	const auto missionMaxVal = CNewBattlePassManager::instance().GetMissionValue(idx);

	if (vec.first + value >= missionMaxVal) {
		vec.first = missionMaxVal;
		vec.second = true;
		CNewBattlePassManager::instance().EndMission(this, idx);
	}
	else {
		vec.first += value;
	}
	CNewBattlePassManager::SendPlayerValue(this, idx, vec.first);
}

void CHARACTER::SaveBattlePassInfo() {
	if (!IsPC()) { return; }
	char file_name[256+1];
	snprintf(file_name, sizeof(file_name), "%s/battlepass_players/%s.json", LocaleService_GetBasePath().c_str(), GetName());
	...
	std::ofstream ofs(file_name, std::ios::trunc | std::ios_base::out);

	try {
		json result = json::array();
		for (BYTE idx = 1; idx <= m_BattlePassInfo.vecBattlePass.size(); ++idx) {
			char szMissionKey[12]{};
			snprintf(szMissionKey, sizeof(szMissionKey), "mission_%u", idx);

			json j;
			j[szMissionKey] = m_BattlePassInfo.vecBattlePass[idx - 1].first;
			result.push_back(j);
		}
		...
		sys_err("SaveBattlePassInfo : %s", e.what());
	}
}
...
void CHARACTER::EndBattlePassMission(const BYTE idx) {
	...
	CNewBattlePassManager::SendPlayerValue(this, idx, vec.first);
	CNewBattlePassManager::instance().EndMission(this, idx);
	ChatPacket(1, "Battle Pass gorevi tamamlandi!");
}
#endif
...
bool CHARACTER::HaveAnotherPagesOpen()
{
	if (GetExchange() ||
		...
		//GetOfflineShopGuest() ||
		//GetShopSafebox() ||
		DragonSoul_RefineWindow_GetOpener())
		return true;
